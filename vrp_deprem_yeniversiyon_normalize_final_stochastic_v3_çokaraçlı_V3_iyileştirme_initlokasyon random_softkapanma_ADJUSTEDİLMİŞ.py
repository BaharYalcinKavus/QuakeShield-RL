...
# === Bulunduğu klasöre kaydetmek için taban dizin ===
# .py dosyası olarak çalışıyorsa __file__ var; notebook/REPL ise CWD kullanılacak.
BASE_DIR = Path(__file__).resolve().parent if '__file__' in globals() else Path.cwd()

# Kaydedilecek dosya adları (bulunduğu klasörde)
MODEL_H5          = BASE_DIR / "cit_q_network_kcekmece.h5"
REWARDS_TRAIN_TXT = BASE_DIR / "cash_in_transit_rewards_train_5.txt"
REWARDS_TEST_TXT  = BASE_DIR / "cash_in_transit_rewards_test_5.txt"
...
PRIORITY_NODES = [316, 464, 607, 748]  # "esas" nodelar
PRIORITY_MIN   = 2                     # her epizotta en az bu kadar "esas node"
DEMAND_QTY_PER_NODE = 1                # node başına adet

# ---------- Shield annealing ----------
SHIELD_ANNEAL = True
SHIELD_ANNEAL_END_EP = 5000
SHIELD_ANNEAL_MIN_FRAC = 0.20  # eğitim sonunda shield kullanma olasılığı hedefi
...
RISK_R_PRIMARY_RANGE    = (0.42, 0.48)   # önce denenir (loglarından türetilmiş öneri)
RISK_R_FALLBACK_RANGE   = (0.30, 0.60)   # sonra denenir
REOPEN_ON_DEMAND        = True
REOPEN_LIMIT_PER_TARGET = 10
FORCE_SYMMETRY          = True
...
for episode in range(num_episodes):
    ...
    # --- EPİZOT BAŞI: Risk uygula (HEDEFLİ KAPAMA + LOG) ---
    total_edges_undir = int(np.sum(np.triu((dst_initial > 0), k=1)))  # tekil kenar (i<j)
    accepted = False
    tries = 0
    ...
    if accepted:
        ...
        if CONNECTIVITY_CHECK_EVERY_EPISODE and (episode % CONNECTIVITY_CHECK_PERIOD == 0):
            ...
            if len(unreachable) > 0:
                print("[RISK] (warning) some targets unreachable after acceptance — but training continues.")
    ...
    for vehicle_number, vehicle in enumerate(vehicles):
        ...
        for t in range(max_num_timesteps):
            ...
            # Replay’e yaz
            if action is not None and action_index is not None:
                terminal = float(done)
    ...
    print(f"Episode {episode} | Training total reward: {total_points:.3f} | eps={epsilon:.4f} "
          f"| deliveries={deliveries_done} | backtracks={backtracks}")
    ...
    # Model .h5'i bulunduğu klasöre kaydet
        q_network.save(str(MODEL_H5))

        # Eğitim/test ödül loglarını bulunduğu klasöre kaydet
        with open(REWARDS_TRAIN_TXT, "w", encoding="utf-8") as fp:
            fp.write('\n'.join(str(item) for item in total_rewards))
        with open(REWARDS_TEST_TXT, "w", encoding="utf-8") as fp:
            fp.write('\n'.join(str(item) for item in total_rewards_test))

        print(f"[SAVE] Model: {MODEL_H5}")
        print(f"[SAVE] Train rewards: {REWARDS_TRAIN_TXT}")
        print(f"[SAVE] Test rewards : {REWARDS_TEST_TXT}")
